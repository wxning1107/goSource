...
func main() {
	...
	// main goroutine 实际上就是代表用户的 main 函数，它都执行完了，肯定是用户的任务都执行完了，直接退出就可以了，就算有其他的 goroutine 没执行完，同样会直接退出。
	exit(0)
...
func schedinit() {
	...
	// getg 由编译器实现，返回当前正在运行的 goroutine 的指针，它会从 tls 里取出 tls[0]，也就是当前运行的 goroutine 的地址
	_g_ := getg()
	if raceenabled {
...
func goexit0(gp *g) {
	...
	// 解除 g 与 m 的关系，其实就是设置 g->m = nil, m->currg = nil
	dropg()
	...
	// 将 g 放入 free 队列缓存起来，供下次创建 g 时快速获取而不用从内存分配，freeg 就是 g 的一个对象池
	gfput(_g_.m.p.ptr(), gp)
	if locked {
...
//go:nosplit
//go:nowritebarrierrec
// 保存调度信息到 g0.sched 结构体
func save(pc, sp uintptr) {
	_g_ := getg()

	// pc指向 gosave 函数返回后的下一条指令
	_g_.sched.pc = pc
	// sp指向 mstart1 函数栈上参数的位置
	_g_.sched.sp = sp
	_g_.sched.lr = 0
	_g_.sched.ret = 0
...
// 启动了一个 goroutine 的时候，在 Go 编译器的作用下，go func(){}语句最终会转化成 newproc 函数，
// 因此，newproc 函数需要两个参数：一个是新创建的 goroutine 需要执行的任务，也就是 fn，它代表一个函数 func。还有一个是 fn 的参数大小。
// 每个 goroutine 都有自己的栈空间，newproc 函数会新创建一个新的 goroutine 来执行 fn 函数，在新 goroutine 上执行指令，就要用新 goroutine 的栈。
// 而执行函数需要参数，这个参数又是在老的 goroutine 上，所以需要将其拷贝到新 goroutine 的栈上。拷贝的起始位置就是栈顶，这好办，那拷贝多少数据呢？由 siz 来确定。
...
func newproc1(fn *funcval, argp unsafe.Pointer, narg int32, callergp *g, callerpc uintptr) *g {
	...
	newg := gfget(_p_)
	if newg == nil {
		// new 一个 g 结构体对象，然后从堆上为其分配 2KB 大小的栈，并设置 g 的 stack 成员和两个 stackgard 成员
		newg = malg(_StackMin)
		// 初始化 g 的状态为 _Gdead
	...
	// 当 goroutine 被调度到 m 上运行时，需要通过 sp 字段来指示栈顶的位置，这里设置的就是新栈的栈顶位置。
	newg.sched.sp = sp
	newg.stktopsp = sp
	...
	// 因此 g0 的 id 是 0，而 main goroutine 的 id 就是 1
...
// 4.把除了 allp[0] 之外的所有 p 放入到全局变量 sched 的 pidle 空闲队列之中，代码里是将所有空闲的 P 放入到调度器的全局空闲队列，对于非空闲的 P（本地队列里有 G 待执行），则是生成一个 P 链表，返回给 procresize 函数的调用者。
func procresize(nprocs int32) *p {
	...
	// 初始化了一个“随机分配器”，将来有些 m 去偷工作的时候，会遍历所有的 P，这时为了偷地随机一些，就会用到 stealOrder 来返回一个随机选择的 P
...
func runqput(_p_ *p, gp *g, next bool) {
	...
	retryNext:
		oldnext := _p_.runnext
		// 用原子操作 cas 来试图将 runnext 成员设置成 newg，目的是防止其他线程在同时修改 runnext 字段。
...
retry:
	...
	t := _p_.runqtail
	// 如果 P 的本地队列没有满，入队，写到队列尾部，同时修改队列尾部的指针。
...
func runqputslow(_p_ *p, gp *g, h, t uint32) bool {
	// 数组长度为 len(_p_.runq)/2+1，也就是 runq 的一半加上 newg。
	...
	// 将从 runq 的头部开始的前一半 goroutine 存入 bacth 数组
	for i := uint32(0); i < n; i++ {
		batch[i] = _p_.runq[(h+i)%uint32(len(_p_.runq))].ptr()
	}
	// 使用原子操作尝试修改 P 的队列头，因为出队了一半 goroutine，所以 head 要向后移动 1/2 的长度。
...
func (q *gQueue) pushBackAll(q2 gQueue) {
	...
	if q.tail != 0 {
		// 如果全局的队列尾 sched.runqtail 不为空，则直接将其和前面生成的链表头相接
		q.tail.ptr().schedlink = q2.head
	} else {
		// 否则说明全局的可运行列队为空，那就直接将前面生成的链表头设置到 sched.runqhead
		q.head = q2.head
	}
	q.tail = q2.tail
}
